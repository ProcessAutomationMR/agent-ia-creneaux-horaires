...
app.post('/occupied-slots', (req, res) => {
 ...
 return res.status(400).json({ message: "Invalid inpu
t, 'value' is required and should contain slots." });
 }
 // Récupérer la date à partir du premier créneau pour
la plage horaire de travail
 const date = occupiedSlots[0].start.split("T")[0];
 const workDayStart = new Date(`${date}T${WORKDAY_STAR
T}Z`);
 const workDayEnd = new Date(`${date}T${WORKDAY_END}Z
`);
 // Trier les créneaux occupés par ordre croissant de d
ébut
...
rrentTime;
 ...
 // Vérifier s'il y a un créneau libre après le dernier
créneau occupé
...
app.post('/suggest-slots', (req, res) => {
 ...
 if (!free_slots || !Array.isArray(free_slots) || free_
slots.length === 0) {
 return res.status(400).json({ message: "Invalid inpu
t, 'free_slots' is required and should contain an array
of slots." });
 }
...
ie
 ...
 return res.status(400).json({ message: "Invalid inpu
t, 'requested_datetime' is required." });
 }
...
`); // Ajouter 'Z' pour spécifier que c'est UTC
 ...
 return res.status(400).json({ message: "Invalid inpu
t, 'requested_datetime' must be a valid ISO date without
timezone." });
 }
 // Étendre de +1 jour
 requestedDate.setUTCDate(requestedDate.getUTCDate() +
1);
 // Si J+1 tombe un samedi ou un dimanche, avancer jusq
u'au lundi
 while (requestedDate.getUTCDay() === 6 || requestedDat
e.getUTCDay() === 0) {
 requestedDate.setUTCDate(requestedDate.getUTCDate()
+ 1);
 }
 // Construire les créneaux d'ouverture (08:00 à 16:00)
pour la nouvelle date
 const year = requestedDate.getUTCFullYear();
 const month = String(requestedDate.getUTCMonth() + 1).
padStart(2, '0');
 const day = String(requestedDate.getUTCDate()).padStar
t(2, '0');
...
// Ajouter cette partie à votre fichier `index.js`
// Route pour convertir les créneaux en UTC+1 et générer
...
s) || suggested_slots.length === 0) {
 return res.status(400).json({ message: "Invalid inpu
t, 'suggested_slots' is required and should contain an a
rray of slots." });
 }
 // Initialiser une chaîne pour construire la réponse e
n français
 let responseText = '';
 // Parcourir chaque créneau, le convertir en UTC+1, et
formater la phrase
 suggested_slots.forEach((slot, index) => {
 const startDate = new Date(slot.start);
 const endDate = new Date(slot.end);
 // Convertir les heures de UTC à UTC+1
 const startUTCPlus1 = new Date(startDate.getTime() +
60 * 60 * 1000);
 const endUTCPlus1 = new Date(endDate.getTime() + 60
* 60 * 1000);
 // Extraire les informations nécessaires pour la phr
ase
 const day = String(startUTCPlus1.getUTCDate()).padSt
art(2, '0');
 const month = startUTCPlus1.toLocaleString('fr-FR',
{ month: 'long' });
 const startHour = startUTCPlus1.getUTCHours();
 const endHour = endUTCPlus1.getUTCHours();
 // Construire la phrase en français
 if (index === 0) {
 responseText += `le ${day} ${month} de ${startHou
r} heures à ${endHour} heures`;
 } else {
 responseText += ` et de ${startHour} heures à ${en
dHour} heures`;
 }
 });
